...
export const Ceil: React.FC<CeilType> = (props) => {
    ...
    const [toggle, FormModal] = useFormModal({
        submit:
            (values) => {
                return editCourseTable({ id, teacher_id: props.teacher_id, timeSlot: content.timeSlot, date: content.date, name: values.name, isChangeName: true, status: content.status }).then(() => {
                })
            },
        title: t('course info'),
        formItems,
