...
const App: React.FC<dynamic.ComponentProps> = (props) => {
    ...
    const getRate = (record: any) => {
        ...
        return `${Math.round((record.trial_class_scheduling_test_sum / record.trial_class_count_sum * 100))}%`
    ...
    const getExperientialRate = (record: any) => {
        ...
        return `${Math.round((record.formal_class_test_sum / record.trial_class_scheduling_test_sum * 100))}%`
    }
    ...
    return (
        <>
            <div style={{ padding: 10 }}>
                <DatePicker defaultValue={dayjs(dayjs().format('YYYY-MM'))} onChange={onChange} picker="month" />
            </div>
            <BaseTable
                rowKey="id"
                columns={columns}
